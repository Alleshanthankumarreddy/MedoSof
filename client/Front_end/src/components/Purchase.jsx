  ...
  return (
    <div className="pt-16 sm:pt-20 md:pt-24 min-h-screen bg-gradient-to-b from-gray-50 to-white w-full">
      <div className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
          ...
          <div className="bg-white p-4 sm:p-6 rounded-2xl border border-gray-200 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl sm:text-3xl font-bold text-gray-900">{lowStockMedicines.length}</p>
              </div>
          ...
          <div className="bg-white p-4 sm:p-6 rounded-2xl border border-gray-200 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl sm:text-3xl font-bold text-gray-900">{filteredMedicines.length}</p>
              </div>
        ...
        {/* Medicine Grid - Mobile Optimized */}
        {filteredMedicines.length === 0 ? (
          <div className="text-center py-16 px-4">
            <div className="inline-flex p-4 bg-gray-100 rounded-full mb-4 mx-auto w-16 h-16 sm:w-20 sm:h-20 flex items-center justify-center">
              {searchTerm ? (
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 mb-10">
              {filteredMedicines.map((med) => (
                <div
                  ...
                  className="group bg-white rounded-2xl border border-gray-200 hover:border-amber-200 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 p-4 sm:p-5 flex flex-col justify-between relative overflow-hidden min-h-[260px] sm:min-h-[280px]"
                >
                  {/* Low Stock Badge */}
                  <div className="absolute top-3 sm:top-4 left-3 sm:left-4">
                    <div className="px-2 sm:px-3 py-1 bg-gradient-to-r from-red-500 to-amber-500 text-white text-xs font-bold rounded-full flex items-center gap-1">
                  ...
                  {/* Medicine Info */}
                  <div className="space-y-3 sm:space-y-4 pt-10 sm:pt-8">
                    {/* Medicine Name */}
                    <div className="flex items-start gap-2 sm:gap-3">
                      <div className="p-1.5 sm:p-2 bg-blue-100 rounded-lg text-blue-600 shrink-0 mt-1">
                    ...
                    <div className="space-y-2 sm:space-y-3">
                      ...
                      <div className="flex items-center justify-between text-xs sm:text-sm">
                        <span className="text-gray-600 truncate">Rack</span>
                        <span className="font-bold text-gray-900 truncate">{med.rackCode}</span>
                      </div>
                    </div>
                  ...
                  <div className="mt-4 sm:mt-6 flex flex-col sm:flex-row gap-2">
                    <button
                      ...
                      className="flex-1 bg-gradient-to-r from-green-500 to-emerald-600 text-white py-2.5 sm:py-2.5 px-3 rounded-xl font-semibold text-sm hover:shadow-lg hover:shadow-green-500/25 transition-all duration-300 flex items-center justify-center gap-2 group/order"
                    >
                      <Plus className="w-4 h-4 group-hover/order:scale-110 transition-transform" />
                      <span className="hidden sm:inline">Order Arrived</span>
                      <span className="sm:hidden">Order</span>
                    </button>
                    <button
                      onClick={() => handleRemoveMedicine(med.medicineCode)}
                      className="flex-1 px-3 py-2.5 sm:py-2.5 bg-gradient-to-r from-gray-500 to-gray-600 text-white rounded-xl font-semibold text-sm hover:shadow-lg hover:shadow-gray-500/25 transition-all duration-300 flex items-center justify-center gap-2 group/cancel"
                    >
            ...
            <div className="bg-white rounded-2xl p-4 sm:p-6 border border-gray-200 shadow-sm">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                  <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-1">Purchase Summary</h3>
                  <p className="text-xs sm:text-sm text-gray-600">
                    Showing {filteredMedicines.length} low-stock medicines
                  </p>
                </div>
        ...
        {/* Batch Entry Modal - Fully Responsive */}
        {selectedMedicine && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[9999] p-4">
            <div className="bg-gradient-to-br from-white via-gray-50 to-white rounded-2xl sm:rounded-3xl shadow-2xl p-6 sm:p-8 w-full max-w-sm sm:max-w-md mx-2 sm:mx-auto animate-scaleIn">
              {/* Close Button */}
              <button
                onClick={handleCloseModal}
                className="absolute top-3 sm:top-4 right-3 sm:right-4 text-gray-400 hover:text-gray-700 hover:bg-gray-100 w-8 h-8 rounded-full flex items-center justify-center transition-all duration-200"
              >
                <X className="w-5 h-5" />
              </button>
              ...
              <div className="text-center mb-6 sm:mb-8">
                <div className="w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-r from-green-500 to-emerald-500 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg">
                ...
                <div className="flex items-center justify-center gap-2 text-xs sm:text-sm text-gray-500">
                  ...
                  <span className="font-medium truncate">{selectedMedicine.medicineName}</span>
                </div>
              </div>
              ...
              <div className="space-y-4 sm:space-y-6">
                ...
                <div className="group">
                  <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-2 ml-1">
                    <span className="flex items-center gap-2">
                      <Hash className="w-3 h-3 sm:w-4 sm:h-4 text-blue-500" />
                      Batch Code
                    </span>
                  </label>
                  <input
                    ...
                    className="w-full px-4 py-3 bg-white border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#0C2C47]/30 focus:border-[#0C2C47] transition-all duration-300 text-sm placeholder-gray-400"
                  />
                </div>
                ...
                <div className="group">
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    className="w-full px-4 py-3 bg-white border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#0C2C47]/30 focus:border-[#0C2C47] transition-all duration-300 text-sm"
                  />
                </div>
                ...
                <div className="group">
                  ...
                  <p className="text-xs text-gray-500 mt-2 ml-1">
                    Current: {selectedMedicine.totalQuantity} • Threshold: {selectedMedicine.threshold}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-6 sm:mt-8 flex gap-3">
                <button
                  onClick={handleCloseModal}
                  className="flex-1 py-3 px-4 bg-gradient-to-r from-gray-500 to-gray-600 text-white font-semibold rounded-xl hover:shadow-lg transition-all duration-300 text-sm"
                >
                  Cancel
                </button>
                <button
                  ...
                  disabled={loading}
                  className="flex-1 py-3 px-4 bg-gradient-to-r from-[#0C2C47] to-[#0A243A] text-white font-semibold rounded-xl hover:shadow-lg transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] text-sm disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2 group/btn"
                >
                  {loading ? (
                    <>
