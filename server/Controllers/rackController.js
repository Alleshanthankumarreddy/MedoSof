...
const showAllRacks = async (req, res) => {
    try {
      ...
      const existingShop = await ownerModel.findOne({ shopCode });
      if (!existingShop) {
        return res.status(404).json({ success: false, message: "Shop not found" });
      }
  
      const racks = await rackModel.find({ shopCode });
      return res.status(200).json({ success: true, racks, message: "Racks fetched successfully" });
    } catch (error) {
...
export { addRack, deleteRack, showAllRacks };
